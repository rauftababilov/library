...
export function EditBookDialog({ open, onClose, onSuccess, initialValues }) {
  const [bookAuthors, setBookAuthors] = useState([]);
  const [bookPublisher, setBookPublisher] = useState(null);
  const [title, setTitle] = useState();
  const queryClient = useQueryClient();
  const createBookMutation = useMutation((newBook) => editBook(newBook), {
    onSuccess: () => {
      onClose();
  ...
  useEffect(() => {
    ...
    setBookAuthors(initialValues.authors);
    setBookPublisher(initialValues.publishingHouse);
